...
try:
    ...
    settail.set(int(config["settings"]["settail"]))
except:
...
def write_settings():
    ...
    config.set("settings","settail",str(settail.get()))
...
lbl_width=Label(frm_width,text="Канальный шаг:")
lbl_width.pack()
...
lbl_rx_freq.pack()
ent_rx_freq.pack()
lbl_volume=Label(frm_rx,text="Уровень принимаемого сигнала (громкости) [0-8] (8-макс.громкость):")
lbl_volume.pack()
ent_volume=Entry(frm_rx)                  
ent_volume.insert(0,volume)
ent_volume.pack()
lbl_squelch=Label(frm_rx,text="Уровень шумоподавителя (SQUELCH) [0-8] (0-всегда вкл.):")
lbl_squelch.pack()
...
chb_settail=Checkbutton(frm_tx,text="Сигнал закрытия ретранслятора:",variable=settail)
